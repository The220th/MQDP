...
Нужно указать один или несколько правильных ответов в третьем столбце.
...
Вполне может быть несколько ответов. Тогда можно явным образом указать это. Нужно дописать проценты к каждому варианту ответа образов.
...
- Процент `%`. Например, если нужно, чтобы ответ был от `3.1415 - 0.0005` *включительно* до `3.1415 + 0.0005` *включительно*, то можно искользовать такую запись: `3.1415 % 0.0005`.
...
После того, как программа отработает, в указанной выходной директории появятся файлы:

- `zip-файл` - файл для **Moodle** `GIFT media`. Загрузите его в  **Moodle**. 

![](./imgs/k_howloaded.png)

- `gift_standartk.txt` - файл для обычного `GIFT`. Если в вопросах не было картинок/рисунков, то можно загрузить этот файл, "как обычный `GIFT`". То есть без плагина [`GIFT media`](https://docs.moodle.org/311/en/Gift_with_medias_format).

- `debug_standartk.txt` - файл с дебаг-информацией

# Если произошла ошибка

Если программа не отработала, как положено, или произошла другая проблема/ошибка, то укажите об этом в [issues](https://github.com/The220th/MQDP/issues): [https://github.com/The220th/MQDP/issues](https://github.com/The220th/MQDP/issues). 

Также для быстрейшего выявления проблемы можно прикрепить файл `debug_standartk.txt`. Учтите, что весь список вопросов и ответов в дебаг-файле `debug_standartk.txt` будет полностью видно. 
